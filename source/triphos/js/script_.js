...
window.addEventListener('scroll', function(){
    ...
    // let currentIndex = 0;
    // historyList.forEach((group, index) => {
    //     const groupTop = group.offsetTop - 200;

    //     if(currentScrollY >= groupTop) {
    //         currentIndex = index;
    //     }
    // });
    // yearList.forEach((button,index) => {
    //     (index === currentIndex) ? button.classList.add('active') : button.classList.remove('active');
    // });
...
//     if(cursor.classList.contains('cursor-grow')) {
//         cursor.classList.remove('cursor-grow');
...
// const subVisual = new IntersectionObserver((entires, observer) => {
//     entires.forEach(entry => {
//         if(entry.isIntersecting) {
...
//subVisualList.forEach(el => subVisual.observe(el));

yearBtn && yearBtn.addEventListener('click', function(e){    
    currentYear = e.target.dataset.group;
    //let offset = document.querySelector(`#${currentYear}`).offsetTop - 230;
    ...
    console.log(selectedYear, selectedGroup)
});
...
siList.addEventListener('click', function(e){
